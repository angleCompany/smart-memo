...
| 自動アップデート | 起動時に新バージョンを確認、通知＋ワンクリックダウンロード |
...
> [!IMPORTANT]
> **⚠️「開発元が未確認」または「アプリが壊れているため開けません」の警告が出る場合**
>
> 本アプリはオープンソースで Apple の有料署名・公証（notarization）がないため、別の Mac でダウンロードして初回起動する際に macOS Gatekeeper が警告を表示します。**ファイルが実際に壊れているわけではありません。**「壊れているため開けません」と出る場合（特に Apple Silicon）は、下の **方法 B（ターミナル）** が最も確実です — このとき方法 A（右クリックで開く）は表示されないことがあります。
>
...
- アプリ起動時に GitHub の最新リリースを自動確認 → 新バージョンがあれば下部に通知バナーを表示
- 設定 → **🔄 アップデート** で現在のバージョン確認と手動 **「アップデート確認」**
- **ダウンロード** をクリック → お使いの Mac（arm64/x64）に合う `.dmg` を取得
- 未署名のオープンソースアプリのため、`.dmg` を開いてアプリを移動し、初回は **右クリック → 開く** で起動してください
...
│   │   └── updateService.js       # アップデート確認（バージョン比較）
...
│   │   └── updateChecker.js    # GitHub リリース取得
