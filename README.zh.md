...
> [!IMPORTANT]
> **⚠️ 出现"未验证的开发者"或"应用已损坏，无法打开"警告时**
>
> 本应用为开源应用，没有 Apple 付费签名/公证（notarization），因此在另一台 Mac 上下载后首次运行时，macOS Gatekeeper 会显示警告。**文件并未真正损坏。** 若提示"应用已损坏，无法打开"（Apple Silicon 常见），请使用下方 **方法 B（终端）**，最为可靠 —— 此时方法 A（右键打开）可能不会出现。
>
...
- 应用启动时自动检查 GitHub 最新发行版 → 有新版本时在底部显示提示横幅
- 设置 → **🔄 更新** 查看当前版本并手动 **"检查更新"**
- 点击 **下载** → 获取与你 Mac（arm64/x64）匹配的 `.dmg`
- 由于是未签名的开源应用，请打开 `.dmg` 移动应用后，首次启动用 **右键 → 打开**
...
│   │   └── updateChecker.js    # 获取 GitHub 发行版
│   └── ui/              # 浏览器渲染器（ES Module）
│       ├── state.js
│       ├── categories.js
│       ├── utils.js
